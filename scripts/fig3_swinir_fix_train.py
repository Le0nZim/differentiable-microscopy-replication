...
Examples:
  # one cell
  python scripts/fig3_swinir_fix_train.py --config <cfg.yaml> --comps x256 \
      --patterns random_fixed --device cuda:0
  # a whole config across cells (sequential)
  python scripts/fig3_swinir_fix_train.py --config <cfg.yaml> --device cuda:0
"""
...
def main() -> None:
    ...
    ap.add_argument("--out-root", default=None, help="default: experiments/figure3_bbbc022_swinir_fix_v1/<config-name>")
    ...
    out_root = Path(args.out_root) if args.out_root else \
        ROOT / "experiments/figure3_bbbc022_swinir_fix_v1" / cfg["name"]
