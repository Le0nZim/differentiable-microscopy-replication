...
PY=./.venv/bin/python
EXP=experiments/figure10_bbbc022_ablation_v1
RUNS=$EXP/runs
mkdir -p "$RUNS"
...
# GPU0 chain: A then B
(
  $PY scripts/fig10_ablation_train.py --variants A --device cuda:0 --out "$RUNS" > "$EXP/train_A.log" 2>&1
  echo "A_exit=$?" >> "$STATUS"
  $PY scripts/fig10_ablation_train.py --variants B --device cuda:0 --out "$RUNS" > "$EXP/train_B.log" 2>&1
  echo "B_exit=$?" >> "$STATUS"
) &
P0=$!
...
# GPU1 chain: C then D
(
  $PY scripts/fig10_ablation_train.py --variants C --device cuda:1 --out "$RUNS" > "$EXP/train_C.log" 2>&1
  echo "C_exit=$?" >> "$STATUS"
  $PY scripts/fig10_ablation_train.py --variants D --device cuda:1 --out "$RUNS" > "$EXP/train_D.log" 2>&1
  echo "D_exit=$?" >> "$STATUS"
) &
P1=$!
...
$PY scripts/fig10_ablation_train.py --aggregate-only --out "$RUNS" >> "$STATUS" 2>&1
$PY scripts/reproduce_fig10.py --runs "$RUNS" >> "$STATUS" 2>&1
