...
for c,res in [('wswinir',256),('transpose256',256),('wcnn64',64)]:
    p=torch.load(f'experiments/figure89_mcf7_swinir_highres_fix_v1/runs_sp1/{c}/illumination/patterns.pt',map_location='cpu')[0,0]
...
$PY scripts/reproduce_fig8.py --config configs/reproduce_fig8_bbbc021_tubulin_x16_swinir.yaml \
    --device cuda:0 --runs-dir "$RUNS" --indices 12 6 4 \
    --out results/reproduced_figures/fig8_sp1
$PY scripts/reproduce_fig9.py --config configs/reproduce_fig9_bbbc021_tubulin_x16_swinir_vs_cnn.yaml \
...
R='experiments/figure89_mcf7_swinir_highres_fix_v1'
