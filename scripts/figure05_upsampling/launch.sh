...
cd "$(dirname "$0")/../.."
PY=.venv/bin/python
OUT=experiments/figure05_upsampling
LOGS=logs/fig05_fixed
mkdir -p "$LOGS" "$OUT"

# GPU 0: the two heaviest 512 columns.
$PY scripts/figure05_upsampling/run.py --device cuda:0 \
...
(
  $PY scripts/figure05_upsampling/run.py --device cuda:1 \
...
$PY scripts/figure05_upsampling/run.py --collect-only --output-root "$OUT"
