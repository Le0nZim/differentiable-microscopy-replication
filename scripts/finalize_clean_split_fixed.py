...
NOISE_OUT = ROOT / "experiments/noise_robustness/rr1_v3_normalized_full_fixed"
SWIN_OUT = ROOT / "experiments/swinir_or_highres/am4_swinir_table2_resolution_fixed/full"
...
def write_fig06(B) -> None:
    d = FIG_DIR / "figure06_noise_robustness_fixed"
...
def write_fig07(B, meta: dict) -> None:
    d = FIG_DIR / "figure07_swinir_standard_sr_fixed"
...
def write_table01() -> None:
    ...
    d = TAB_DIR / "table01_noise_robustness_fixed"
    ...
    src_paper = TAB_DIR / "table01_noise_robustness_fixed/paper_values.csv"
...
def write_table02(summary: dict) -> None:
    d = TAB_DIR / "table02_swinir_sr_fixed"
    (d / "rendered").mkdir(parents=True, exist_ok=True)
    (d / "components").mkdir(parents=True, exist_ok=True)
    ...
    with (d / "comparison.csv").open("w", newline="", encoding="utf-8") as h:
        ...
        w.writeheader()
        w.writerows(cmp_rows)
    src_paper = TAB_DIR / "table02_swinir_sr_fixed/paper_values.csv"
    if src_paper.exists() and src_paper.resolve() != (d / "paper_values.csv").resolve():
        shutil.copy2(src_paper, d / "paper_values.csv")
...
def render_fig07_full_images() -> dict:
    ...
    spec = importlib.util.spec_from_file_location(
        "fig7_full",
        ROOT / "scripts/render_fig07_full_image_eval.py",
    )
    mod = importlib.util.module_from_spec(spec)
    # Point the renderer at the clean-split run.
    spec.loader.exec_module(mod)
    mod.CONFIG = REPL / "configs/swinir/am4_table2_full_fixed.yaml"
    mod.RUN = SWIN_OUT
...
def main() -> None:
    ...
    B = _load_build()
    write_fig06(B)
    print("Wrote figure06_noise_robustness_fixed", flush=True)
    write_table01()
    print("Wrote table01_noise_robustness_fixed", flush=True)
    meta = render_fig07_full_images()
    write_fig07(B, meta)
    print("Wrote figure07_swinir_standard_sr_fixed", flush=True)
    summary = json.loads((SWIN_OUT / "aggregate_summary.json").read_text())
    write_table02(summary)
    print("Wrote table02_swinir_sr_fixed", flush=True)
