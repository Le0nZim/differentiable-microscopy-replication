...
``experiments/figure10_bbbc022_ablation_v1/configs/fig10_ablation_base.yaml``.
...
# Reuse the validated ablation protocol verbatim.
from run_am3_table3 import (  # noqa: E402
    VARIANT_LABEL,
    apply_variant,
    default_phases,
    run_one,
...
EXP = ROOT / "experiments/figure10_bbbc022_ablation_v1"
CFG = EXP / "configs/fig10_ablation_base.yaml"
...
def main() -> None:
    ...
    print(f"Fig10 ablation: variants={args.variants} device={args.device} "
          f"scale={args.scale} cuda={torch.cuda.is_available()}", flush=True)
    print(f"  phases (per variant): "
          f"{[f'{p['name']}:{p['steps']}' for p in phases]}", flush=True)
