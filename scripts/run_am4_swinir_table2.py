...
verified by scripts/audit_am4_swinir_fairness.py).

Usage:
  # full pipeline (both conditions sequentially, then aggregate + figures)
  python scripts/run_am4_swinir_table2.py --config configs/swinir/am4_table2_smoke.yaml

  # one condition on a specific GPU (parallelise across GPUs), then aggregate
  python scripts/run_am4_swinir_table2.py --config configs/swinir/am4_table2_budget.yaml \
  ...
  python scripts/run_am4_swinir_table2.py --config configs/swinir/am4_table2_full.yaml --resume
"""
