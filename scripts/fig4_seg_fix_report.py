...
``experiments/figure4_bbbc022_segmentation_fix_v1/runs/`` and produces:

  * ``metrics/fig4_metrics.csv``          - per-cell Dice/IoU/threshold table
  * ``metrics/fig4_vs_am2.md``            - cross-check vs. the frozen am2 run
    (note: different pseudo-GT mask recipe — TrackMate here vs. thr0.3+closing in am2)
...
Runs only against checkpoints produced by scripts/fig4_seg_fix_train.py; the
frozen am2 run is read for comparison only. No test-set tuning: the per-cell
...
_spec = importlib.util.spec_from_file_location("fig4run", ROOT / "scripts/fig4_seg_fix_train.py")
...
EXP = ROOT / "experiments/figure4_bbbc022_segmentation_fix_v1"
RUNS = EXP / "runs"
FIGS = EXP / "figures"
METRICS = EXP / "metrics"
...
def write_vs_am2(seed: int) -> Path:
    ...
    lines = ["# Fig4 fix vs. frozen am2 (cross-check)\n\n",
             ...
             "`am2_task_aware_full` run used the legacy A.2.3 (normalize→thr 0.3→closing 10) "
...
def update_report(seed: int) -> None:
    ...
    concl = (
        ...
        f"Stage 3. The method was already faithful in `am2_task_aware_full`; this "
