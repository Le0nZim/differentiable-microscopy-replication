...
directory ``experiments/figure04_segmentation/task_aware/runs/``. Stage 1
(content-aware pretrain) is REUSED from the frozen
``experiments/figure04_segmentation/stage1_frozen`` checkpoints
(``stage1_mode=load``) so only Stage 2 (frozen seg-head) and Stage 3
...
report. Additive only — the frozen Stage-1 tree is not modified.
"""
...
OUT = ROOT / "experiments/figure04_segmentation/task_aware/runs"
...
def _write_report(results: list[dict]) -> None:
    ...
    lines = [
        ...
        "experiments/figure04_segmentation/stage1_frozen; Stage 2 (frozen seg-head) + Stage 3 (end-to-end task-aware "
