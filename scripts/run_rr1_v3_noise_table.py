...
Everything is written under ``experiments/noise_robustness/rr1_v3_normalized_full/``.
Frozen v1/v2 outputs are never touched.

Usage (two GPUs, then aggregate):

    python scripts/run_rr1_v3_noise_table.py --device cuda:0 --shard 0 --num-shards 2
    python scripts/run_rr1_v3_noise_table.py --device cuda:1 --shard 1 --num-shards 2
    python scripts/run_rr1_v3_noise_table.py --aggregate-only --device cuda:0
"""
...
OUTPUT_ROOT = ROOT / "experiments/noise_robustness/rr1_v3_normalized_full"
CONFIG_PATH = ROOT / "configs/paper_aligned_patchmnist_noise_table_v3.yaml"
...
def _v2_learnable_spreads() -> dict:
    v2_csv = ROOT / "experiments/noise_robustness/noise_table_normalized_v2/results.csv"
    out: dict = {}
