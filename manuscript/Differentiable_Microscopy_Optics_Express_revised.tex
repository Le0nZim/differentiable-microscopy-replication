...
\begin{abstract*}
The trade-off between throughput and image quality is an inherent challenge in microscopy. To improve throughput, compressive imaging under-samples image signals; the images are then computationally reconstructed by solving a regularized inverse problem. Compared with traditional regularizers, deep-learning-based methods have achieved greater success in compression and image quality. However, information lost during acquisition sets the compression bounds, so further increasing compression without compromising reconstruction quality remains challenging. In this work, we propose differentiable compressive fluorescence microscopy ($\partial \mu$), which combines a differentiable, parameterized forward model with learnable physical parameters (e.g., illumination patterns) and a physics-inspired inverse model. The cascaded model is end-to-end differentiable and learns compressive sampling schemes from training data. We evaluate reconstruction and task-aware compression under an explicit sample-count measurement budget, and compare spatial and Fourier parameterizations as optimization choices. The historical numerical estimates in this draft require the controlled reruns identified in the accompanying scientific audit before comparative performance claims can be finalized.

\end{abstract*}

\noindent\textbf{Draft audit status (12 September 2026).} Numerical tables and figures below are historical outputs pending controlled reruns. Code corrections have not regenerated these results. Comparative, binary-deployment, throughput, and biological-accuracy claims must be assessed against the accompanying \texttt{docs/SCIENTIFIC\_AUDIT.md} and rerun protocol.\par
...
Our experiments compare fixed and learned illumination on PatchMNIST and BBBC022 Hoechst widefield images, examine upsampling architectures, and extend the pipeline to SwinIR reconstruction and pseudo-mask segmentation. The scientific audit identified training-budget, evaluation, and acquisition-accounting issues that prevent the historical rankings from establishing the final conclusions. Controlled reruns will compare binary deployed masks, explicit photon constraints, matched update budgets, and fixed validation-selection rules. Full-spectrum Fourier parameterization remains an optimization ablation whose outcome depends on the declared protocol.
