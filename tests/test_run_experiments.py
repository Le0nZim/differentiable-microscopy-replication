...
def test_run_matrix_dry_run():
    summaries = run_matrix("configs/matrices/patchmnist_x8_matrix_debug.yaml", dry_run=True)
...
def test_run_matrix_smoke(tmp_path: Path):
    ...
    matrix_path.write_text(
        Path("configs/matrices/patchmnist_x8_matrix_debug.yaml").read_text(encoding="utf-8"),
        encoding="utf-8",
    )
    base_path = tmp_path / "base_patchmnist.yaml"
    base_path.write_text(
        Path("configs/base_patchmnist.yaml").read_text(encoding="utf-8"),
        encoding="utf-8",
    )
