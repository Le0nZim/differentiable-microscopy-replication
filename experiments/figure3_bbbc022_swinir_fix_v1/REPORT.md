...
**Experiment:** `experiments/figure3_bbbc022_swinir_fix_v1/`
...
- **Trainer:** `scripts/fig3_swinir_fix_train.py` — per-cell training with grad-accum to effective batch 32, bf16, warmup+cosine LR, grad-clip, val every 1k, SSIM-primary best-checkpoint selection, saves `best.pt`/`last.pt`/`config.yaml`/`metrics.json`/val+test grids.
- **Sanity:** `scripts/fig3_swinir_fix_sanity.py` (see §5).
- **Report/figures:** `scripts/fig3_swinir_fix_report.py` — builds `metrics_summary.csv`, the SwinIR diagnostic panel, random-field panels, and (with `--full-panel`) the full 6-column Fig-3.
...
python scripts/fig3_swinir_fix_sanity.py --device cuda:0

# 2. Paper-faithful l1_ssim, all 8 cells (split across GPUs; ~8h/cell at 50k)
python scripts/fig3_swinir_fix_train.py --config experiments/figure3_bbbc022_swinir_fix_v1/configs/paper_faithful_l1_ssim.yaml \
...
python scripts/fig3_swinir_fix_train.py --config experiments/figure3_bbbc022_swinir_fix_v1/configs/paper_faithful_pixel_perceptual_gan.yaml \
    --comps x16,x64  --patterns random_fixed,learnable_frequency --device cuda:0
python scripts/fig3_swinir_fix_train.py --config experiments/figure3_bbbc022_swinir_fix_v1/configs/paper_faithful_pixel_perceptual_gan.yaml \
...
python scripts/fig3_swinir_fix_report.py --name paper_faithful_pixel_perceptual_gan --device cuda:0 --full-panel
```
