...
cd "$(dirname "$0")/../.." || exit 1
PY=python
RUNS=experiments/figure04_segmentation/task_aware/runs
LOG=experiments/figure04_segmentation/task_aware/logs/finalize.log
CELLS=(
...
for _ in $(seq 1 720); do   # up to ~2h (720 * 10s)
  ...
  if [ "$done_count" -eq 6 ]; then
    ...
    CUDA_VISIBLE_DEVICES=0 $PY scripts/figure04_segmentation/report.py --device cuda:0 --seed 42 >> "$LOG" 2>&1
