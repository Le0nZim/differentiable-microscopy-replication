...
implemented faithfully** in `experiments/task_aware_segmentation/am2_task_aware_full/`
...
CUDA_VISIBLE_DEVICES=0 $PY scripts/fig4_seg_fix_sanity.py --device cuda:0
# 2) verified re-run of the 6-cell matrix (Stage 2+3; Stage 1 reused frozen)
CUDA_VISIBLE_DEVICES=0 $PY scripts/fig4_seg_fix_train.py --device cuda:0 --seed 42
# 3) metrics CSV + paper-layout Figure 4 + bars
CUDA_VISIBLE_DEVICES=0 $PY scripts/fig4_seg_fix_report.py --device cuda:0 --seed 42
```
