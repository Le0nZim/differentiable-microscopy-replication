...
(exhaustively documented in `experiments/ablations/am3_table3_resolution/U2OS_DATA_STATUS.md`).
...
# or per-variant
./.venv/bin/python scripts/fig10_ablation_train.py --variants A B C D --device cuda:0

# re-render only (from saved run artifacts)
./.venv/bin/python scripts/reproduce_fig10.py --runs experiments/figure10_bbbc022_ablation_v1/runs
```
